...
const ZILTAG_EMBEDDED_API = 'http://' + STAGING_SERVER + '/embedded/ziltagging/';
...
riot.tag('ziltag-plugin',
    ...
    <div class='ziltag-toolbar'>
        ...
        <div class='ziltag-toolbar-ziltag-stack'>
    ...
    <div class='ziltag-reader-cover'></div>
    <iframe class='ziltag-reader'></iframe>
    `,
    function(opts) {

        this.on('mount', () => {

            let ziltag_reader = document.getElementsByClassName('ziltag-reader')[0];
            let reader_cover = document.getElementsByClassName('ziltag-reader-cover')[0];

            reader_cover.addEventListener('click', () => {
                reader_cover.style.display = 'none';
                ziltag_reader.style.display = 'none';
            });
            ...
            for(let i = 0; i < imgs.length; i++) {
                ...
                fetch(ZILTAG_FETCH_API + `?source=${src}`, {
                    credentials: 'include'
                })
                .then((response) => {
                ...
                .then((ziltags) => {
                    for(let i = 0; i < ziltags.length; i++) {
                        ...
                        ziltag.dataset['src'] = ziltag_url;

                        ziltag.addEventListener('click', () => {
                            reader_cover.style.display = 'block';
                            ziltag_reader.src = ZILTAG_EMBEDDED_API + id;
                            ziltag_reader.style.display = 'block';
                        });

                        img.parentNode.appendChild(ziltag);
                    }
                });
            }
        ...
        let post_login = (json) => {
            ...
            let ziltag = document.getElementsByClassName('ziltag-toolbar-ziltag-stack')[0];
            ziltag.addEventListener('dragstart', (e) => {
